"""
Phases 4-6a -- build the interactive map (output/index.html).

Input:
  data/processed/equity.geojson   (Phase 3, 158 polygons EPSG:4326, scores locked)
  data/processed/access.parquet   (only for total_effective_frequency, the
                                   capacity-weighted + walkshed frequency column
                                   Phase 3 did NOT carry into equity.geojson)
Output:
  output/index.html   one self-contained file: folium/Leaflet map + all its JS/CSS
                      inlined, the Noto Sans font and SheetJS vendored as data URIs,
                      so the published page makes no third-party requests except
                      the basemap tiles (CLAUDE.md section 2). Regenerated by this
                      script; gitignored.

What the page has (CLAUDE.md section 1):
  * an Esri dark-gray basemap centred on Toronto (keyless)
  * THREE choropleth layers, one painted at a time, switched by a custom Layers
    control (folium's own LayerControl is not used):
      - "Equity gap"  equity_gap    diverging indigo->hot-pink, bins symmetric on 0
                                    -- shown by default
      - "Access"      access_score  sequential YlOrRd, 0 -> max
      - "Need"        need_score     sequential YlOrRd, 0 -> max
  * ONE always-on transparent GeoJson layer on top carrying the hover tooltip,
    kept in front on every layer switch via bringToFront()
  * a custom dark-UI shell (title, layers panel, swappable legend, zoom pill,
    licence footer), an "About this map" dialog, and a client-side
    "Download data (.xlsx)" button

folium 0.20 has no templating hook for a surrounding page layout and its
LayerControl markup can't be restyled to this design, so the entire dark UI is
string-injected onto folium's rendered HTML (build_shell / SHELL_CSS / ABOUT_HTML
below). No scoring or data logic lives here -- the scores come straight from
equity.geojson.
...
# choropleth so place names stay readable). Chosen because it is keyless and muted
# enough to sit under a choropleth in the dark UI shell. CartoDB Positron /
# dark_matter -- the usual picks -- now serve an "API KEY REQUIRED" watermark for
# keyless use.
...
# 6-step diverging ramp for the gap layer, neon deep-indigo -> hot-pink: blue =
# over-served, pink = underserved. Bright enough to read on the dark basemap.
# Used for BOTH the gap choropleth and its legend; Access/Need use YlOrRd.
...
#  not reusable for this design -- see the module docstring).
# -----------------------------------------------------------------------------
...
def build_shell(map_name: str, layer_js: dict, tooltip_js: str,
    ...
    # is the alternative but it demands arrow-key roving focus (more JS) and drops
    # the per-button Tab stop.
    rows = "".join(
